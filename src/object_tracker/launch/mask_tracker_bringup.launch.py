...
#   2. params file (params_file argument, default config/mask_tracker_params.yaml)
#   3. NODE_ARG_DEFAULTS below
#   4. declare_parameter defaults inside mask_tracker_node
...
ARGUMENTS = [
    DeclareLaunchArgument(
        "params_file",
        default_value=os.path.join(get_package_share_directory('object_tracker'), 'config', 'mask_tracker_params.yaml'),
    ...
    DeclareLaunchArgument(
        "use_sim_time",
        default_value="false",
        description="Use /clock (bag replay with --clock); passed to mask_tracker_node and mask_init_tool",
    ),

    # Simulated upstream VLM masks
    DeclareLaunchArgument(
        "launch_init_tool",
        default_value="false",
    ...
    DeclareLaunchArgument(
        "debug.enable",
        default_value=UNSET,
        description="Enable debug log and tracked_object TF (unset = params file, else true)",
    ),
    DeclareLaunchArgument(
        "debug.img",
        default_value=UNSET,
        description="Enable image show for debugging (unset = params file, else false)",
    ),
]
...
def launch_setup(context):
    ...
    mask_tracker_node = Node(
        package='object_tracker',
        executable='mask_tracker_node',
        name='mask_tracker_node',
        output='screen',
    ...
    return [camera_static_tf, mask_tracker_node, mask_init_tool]
