...
// Client side of docs/vlm_transport.md: send a JPEG to the VLM server over ZeroMQ and publish the returned
// object region on mask_topic, stamped with the frame it belongs to. The server returns a bbox (and a mask only
// when VLM_RETURN_MASK=1); a bbox-only answer is published as a filled bbox for the trackers to cut out with depth.
// The trackers already consume that topic, so this node needs no changes on their side.
//
// Not implemented (needs tracker state, see docs section 6): "send after N lost frames".
// Networking runs in its own thread, so a stalled link never blocks the trackers.
...
const char *to_string(VlmEvent event) {
    switch (event) {
...
public:
    VlmBridgeNode() : Node("vlm_bridge_node") {
        ...
        this->declare_parameter<bool>("vlm.enable", true);
        this->declare_parameter<std::string>("vlm.endpoint", "tcp://192.168.50.125:5555");
        this->declare_parameter<double>("vlm.timeout_s", 6.0);
        this->declare_parameter<double>("vlm.refresh_period_s", 8.0);
        this->declare_parameter<int>("vlm.upload_max_width", 640);
        this->declare_parameter<int>("vlm.jpeg_quality", 80);
        ...
        vlm_enable_ = this->get_parameter("vlm.enable").as_bool();
        endpoint_ = this->get_parameter("vlm.endpoint").as_string();
        timeout_s_ = this->get_parameter("vlm.timeout_s").as_double();
        refresh_period_s_ = this->get_parameter("vlm.refresh_period_s").as_double();
        upload_max_width_ = std::max(1, static_cast<int>(this->get_parameter("vlm.upload_max_width").as_int()));
        jpeg_quality_ = std::clamp(static_cast<int>(this->get_parameter("vlm.jpeg_quality").as_int()), 1, 100);
        ...
        RCLCPP_INFO(this->get_logger(), "VLM bridge: %s -> %s, masks on %s, refresh %.1f s, timeout %.1f s",
                    color_topic_.c_str(), endpoint_.c_str(), mask_topic.c_str(), refresh_period_s_, timeout_s_);
        if (!vlm_enable_) {
            RCLCPP_WARN(this->get_logger(), "vlm.enable is false: no requests are sent");
            return;
        }
        network_thread_ = std::thread(&VlmBridgeNode::network_loop, this);
    }

    ~VlmBridgeNode() override {
        running_ = false;
...
private:
    ...
    void report_stats() {
        ...
        if (total > 0) {
            ...
            RCLCPP_INFO(this->get_logger(), "VLM stats:%s | query_version=%d |%s", breakdown.c_str(), query_version_,
                        latency.c_str());
        }
    ...
    void network_loop() {
        ...
        while (running_ && rclcpp::ok()) {
            ...
            if (pending_ && std::chrono::duration<double>(now - pending_->sent_at).count() > timeout_s_) {
                RCLCPP_WARN(this->get_logger(), "Request %u timed out after %.1f s", pending_->request_id, timeout_s_);
    ...
    void send_detect(zmq::socket_t &socket) {
        ...
        if (!socket.send(zmq::buffer(header_text), zmq::send_flags::sndmore | zmq::send_flags::dontwait)) {
            RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 5000, "Not connected to %s, retrying",
                                 endpoint_.c_str());
            next_send_at_ = std::chrono::steady_clock::now() + std::chrono::milliseconds(500);
            return;
        }
        if (!send_body(socket, jpeg)) {
            RCLCPP_WARN(this->get_logger(), "Could not send the image of request %u", frame.request_id);
            next_send_at_ = std::chrono::steady_clock::now() + std::chrono::milliseconds(500);
            return;
        }
        ...
        RCLCPP_INFO(this->get_logger(), "Sent request %u: %dx%d, %zu KB, stamp=%.3f", frame.request_id,
    ...
    void receive(zmq::socket_t &socket) {
        ...
        json header;
        try {
            header = json::parse(frames[0].to_string());
        } catch (const json::exception &e) {
            RCLCPP_WARN(this->get_logger(), "Bad response header: %s", e.what());
        ...
        if (version != query_version_) {
            ...
            // inference (about 3.4 s on the GPU)
            if (query_version_ >= 0) {
                RCLCPP_INFO(this->get_logger(), "Query version %d -> %d, sending a new image", query_version_, version);
        ...
        if (!pending_ || response_id != pending_->request_id) {
            ...
            RCLCPP_INFO(this->get_logger(), "Dropping response %u (waiting for %s)", response_id, waiting.c_str());
            return;
        }
        ...
        if (status == "FOUND") {
            if (publish_mask(header, frames, frame)) {
            ...
            RCLCPP_INFO(this->get_logger(), "Request %u: target not in the image", response_id);
        } else if (status == "NO_QUERY") {
            record_event(VlmEvent::NO_QUERY);
            RCLCPP_WARN(this->get_logger(), "Server has no query yet, backing off %.1f s", no_query_backoff_s_);
            ...
            RCLCPP_WARN(this->get_logger(), "Server error: %s, retrying in %.1f s",
    ...
    // and paste into a full-size mono8 mask. Without a mask (the server default) the whole bbox is published:
    // the trackers' mask cleaning then cuts the object out with depth (median depth gate + largest region),
    // which is what docs section 5 asks the client to do.
    bool publish_mask(const json &header, const std::vector<zmq::message_t> &frames, const SentFrame &frame) {
        std::array<int, 4> bbox{0, 0, 0, 0};
        try {
            const auto &values = header.at("bbox");
            if (!values.is_array() || values.size() != 4) {
                RCLCPP_WARN(this->get_logger(), "FOUND without a valid bbox");
                return false;
            }
            ...
            RCLCPP_WARN(this->get_logger(), "Bad bbox: %s", e.what());
            return false;
        }
        const int box_w = bbox[2] - bbox[0];
        const int box_h = bbox[3] - bbox[1];
        if (box_w <= 0 || box_h <= 0) {
            RCLCPP_WARN(this->get_logger(), "Empty bbox [%d, %d, %d, %d]", bbox[0], bbox[1], bbox[2], bbox[3]);
            return false;
        }
        ...
        if (has_mask) {
            if (frames.size() < 2) {
                RCLCPP_WARN(this->get_logger(), "has_mask is true but the mask frame is missing");
                return false;
            }
            ...
            if (mask_roi.empty()) {
                RCLCPP_WARN(this->get_logger(), "Could not decode the mask PNG");
                return false;
            }
            if (mask_roi.size() != cv::Size(box_w, box_h)) {
                RCLCPP_WARN(this->get_logger(), "Mask %dx%d does not match bbox %dx%d", mask_roi.cols, mask_roi.rows,
                            box_w, box_h);
                return false;
            }
        } else {
        ...
        if (target.width <= 0 || target.height <= 0) {
            RCLCPP_WARN(this->get_logger(), "bbox collapses after scaling");
            return false;
        }
        cv::Mat scaled;
        ...
        if (clipped.width <= 0 || clipped.height <= 0) {
            RCLCPP_WARN(this->get_logger(), "bbox is outside the image");
            return false;
        }
        ...
        cv_bridge::CvImage out(frame.header, sensor_msgs::image_encodings::MONO8, mask);
