...
// the output is the median 3D point of the inlier points.
...
public:
    MaskTrackerNode() : Node("mask_tracker_node") {
...
private:
    ...
    // Builds a candidate model; it is tried on the next processed frame (handoff), since only the image
    // callback has a current frame
    ...
    // Median camera-frame point of the inliers with real depth; with too few of them, a default depth along the
    // median pixel when the object looks too far rather than too near. Updates d_obj from real depth only.
